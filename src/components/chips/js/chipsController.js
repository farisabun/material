...
function MdChipsCtrl ($scope, $attrs, $mdConstant, $log, $element, $timeout, $mdUtil,
  ...
  this.containerEmptyHint =
    'Chips container. Type text and then press enter to add a chip.';
   ...
   * @type {string}
   * @deprecated removed in 1.2 as we no longer expose the button in the accessibility tree
   */
...
MdChipsCtrl.prototype.init = function() {
  ...
  // If we're using static chips, then we need to initialize a few things here
  if (!this.$element.attr('ng-model')) {
    // Use different default hints
    if (!this.$element.attr('container-hint')) {
      this.containerHint = 'Use the move to next command to navigate to the first chip.';
    }
    if (!this.$element.attr('container-empty-hint')) {
      this.containerEmptyHint = 'Use the move to next command to navigate to the first chip.';
    }
    // Use different roles and aria attributes
...
MdChipsCtrl.prototype.removeChip = function(index, event) {
  ...
  this.$mdLiveAnnouncer.announce(chipContent + ' ' + this.removedMessage,
    'assertive');
...
MdChipsCtrl.prototype.configureAutocomplete = function(ctrl) {
  if (ctrl) {
    ...
    if (!this.$element.attr('container-empty-hint')) {
      this.containerEmptyHint = 'Chips container with autocompletion. Type text to search. ' +
